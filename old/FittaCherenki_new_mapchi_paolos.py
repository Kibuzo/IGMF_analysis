...
#Esegui fino a riga 257 se sei qui per plotBPL
#ToDo: Prova a fare il fit su un parametro e vedi se ti viene la larghezza consistente con quella della banana
import os
import numpy as np
...
#------------------------------New chi2 calculator--------------------------------#
def Mapchi2(E,dysup,dyinf,fmin,fmax,cutmin,cutmax,gammamin,gammamax,stepf,stepcut,stepg): #metti fmin e fmax e stepf
    a=[]
    f_idx=-1
    ...
    b=dysup/dyinf
    g=(dysup*dyinf)/np.abs(dysup-dyinf)
    ...
    for f in np.arange(fmin,fmax,stepf):
        ...
        for ecut in np.arange(cutmin,cutmax,stepcut):
            ...
            for gamma in np.arange (gammamin,gammamax,stepg):
                ...
                #DYDEASS=DYCHER/(assorbi(XCHER,YCHER,franceschini)/YCHER)
                chi=np.sum((np.log(1+np.abs(residuals)/g)/np.log(b))**2) #Barlow 2003, passato da sofia su messenger
                #chi=np.sum((residuals/((dysup+dyinf)/2))**2) #Caso simmetrizzato
                #if (chi==np.isnan):
                # chi=np.inf
...
def PowerFitAss(Energy,Prefactor,Index):
    Scale=14.731
    return assorbi(Energy,(Prefactor*(Energy/Scale)**Index),franceschini)
...
def plottahess():
    ...
    plt.errorbar(Hessx,deassorbi(Hessx,Hessy,dominguez),yerr=(Hessdyl,Hessdyh),linestyle='',marker='.',markersize=2)
...
Hessdyh=np.array([2.64e-12, 2.52e-12, 1.46e-12, 1.09e-12, 8.09e-13, 7.42e-13, 5.06e-13, 6.96e-13,0,0,0,0])*10**-3
Hessdyh=Hessdyh*(assorbi(Hessx,Hessy,franceschini)/Hessy)
Hessdyl=np.array([5.87e-12,2.35e-12,1.69e-12,9.54e-13,7.11e-13,6.01e-13,3.24e-13,6.13e-13,0,0,0,0])*10**-3
Hessdyl=Hessdyl*(assorbi(Hessx,Hessy,franceschini)/Hessy)
...
Veritasdyh=np.array([1.25e-11, 5.74e-12, 2.42e-12, 2.67e-12, 2.06e-12, 1.27e-12, 1.17e-12])*10**-3
Veritasdyl=np.array([1.2e-11,6.13e-12,3.04e-12,2.15e-12,1.49e-12,1.28e-12,1.15e-12])*10**-3
Veritasdyh=Veritasdyh*(assorbi(Veritasx,Veritasy,franceschini)/Veritasy)
Veritasdyl=Veritasdyl*(assorbi(Veritasx,Veritasy,franceschini)/Veritasy)
...
DYCHERl=np.array(Veritasdyl.tolist()+Hessdyl[:-4].tolist())
DYCHERh=np.array(Veritasdyh.tolist()+Hessdyh[:-4].tolist())
...
XBIN=[3.7050000000000005, 10.175, 19.655, 37.95, 73.27000000000001, 153.27, 254.5, 360.0, 479.5, 565.0, 647.0, 769.0, 885.0, 1057.0, 1214.0, 1459.0, 1671.5, 1966.5, 2639.0, 3538.0, 4578.0] # Se mai volessi usare dei bin personalizzati per istogrammare questi sono gli estremi

#------------Diagnostic plots-------------#
def plottapuntide():
    Y=deassorbi(XCOMBINED,YCOMBINED,dominguez)
    DY=DYCOMBINED*deassorbi(XCOMBINED,YCOMBINED,dominguez)/YCOMBINED
    plt.errorbar(XCOMBINED, Y, yerr=DY,linestyle='',marker='v',markersize=2,label='Experimental data')
    plt.xscale('log')
    plt.yscale('log')
...
def plottacutoff(f,gamma,ebreak):
    if ebreak>10000:
        titolo='Simple power law'
    else:
        titolo='Exponential cutoff'
    plt.plot(XCOMBINED,XCOMBINED*Cutoff(f,XCOMBINED,gamma,ebreak),label=titolo)
...
plt.errorbar(Hessx, Hessy, yerr=(Hessdyl,Hessdyh),linestyle='',marker='v', label='Dati HESS',markersize=5)
...
plt.errorbar(Veritasx, Veritasy, yerr=(Veritasdyl,Veritasdyh),linestyle='',marker='v', label='Dati VERITAS',markersize=5)
...
# plt.close()
# 
# plt.errorbar(Hessx, deassorbi(Hessx,Hessy,dominguez), yerr=(Hessdyl,Hessdyh),linestyle='',marker='v', label='Dati HESS + dominguez EBL',markersize=5)
# plt.errorbar(Veritasx, deassorbi(Veritasx,Veritasy,dominguez), yerr=(Veritasdyl,Veritasdyh),linestyle='',marker='v', label='Dati VERITAS + dominguez EBL',markersize=5)
...
plt.errorbar(Hessx, Hessy, yerr=(Hessdyl,Hessdyh),linestyle='',marker='v', label='Dati HESS + dominguez EBL',markersize=5)
plt.errorbar(Veritasx, Veritasy, yerr=(Veritasdyl,Veritasdyh),linestyle='',marker='v', label='Dati VERITAS + dominguez EBL',markersize=5)
plt.errorbar(Fermix,nphot,yerr=dNphot,marker='.', markersize=2,uplims=fermiuplims,linestyle=''
    ,label='Fermi analysis + dominguez ebl')
XTOT=np.linspace(np.min(Fermix),np.max(Hessx))
...
#senza
plt.errorbar(Hessx, Hessy, yerr=(Hessdyl,Hessdyh),linestyle='',marker='v', label='Dati HESS',markersize=5)
plt.errorbar(Veritasx, Veritasy, yerr=(Veritasdyl,Veritasdyh),linestyle='',marker='v', label='Dati VERITA',markersize=5)
plt.errorbar(Fermix,nphot,yerr=dNphot,marker='.', markersize=2,uplims=fermiuplims,linestyle=''
    ,label='Fermi analysis')
XTOT=np.linspace(np.min(Fermix),np.max(Hessx))
plt.plot(XTOT,LogParabola(XTOT,mflp.values[0],mflp.values[1],mflp.values[2]),label='Fermi logparabola estrapolato', linestyle='--')
...
#ToDo: Modificare questa cosa per avere errori asimmetrici
Mchi2,mappa,power=Mapchi2(XCHER,DYCHERh,DYCHERl,fval[0],fval[1],cutoff[0],cutoff[1],gammaval[0],gammaval[1],step[0],step[2],step[1])
...
#minimi=powerslice[np.where(chislice<minchi+6.25)]
#minimo=np.where()
...
#Artificialmente: il gamma richiesto di 1.5 è nella posizione 63, si sostituisce a dove[1][0]
#il cutoff invece si sostituisce a [2][0] e fa 40 per 2080, un giorno trova il modo di fare bene questa cosa
plt.plot((41,41),(99,64),color='k',linestyle='--')
plt.plot((0,41),(64,64),color='k',linestyle='--')
plt.text(0,64,'$\Gamma$=1.5', color='yellow')
plt.text(41,98,'E$_{cut}$=2080 GeV', color='yellow')

#plt.text(40,66, '$\Gamma$=1.5,'+'\n'+'Ecut=2080GeV', color='r')
...
# Finally show the plot and possibly be happy
plt.savefig(savedir+'Powers_asymm.png')
plt.show()

# # Funzione usata per manda il plot a paolo
# DYASS=DYCHER*(deassorbi(XCHER,YCHER,franceschini)/YCHER)
# poptpl,pcovpl=curve_fit(PowerFit2,XCHER,deassorbi(XCHER,YCHER,franceschini),sigma=DYASS)
# xcherfit=np.linspace(np.min(XCHER), np.max(XCHER),3)
# ycherfit=PowerFit2(XCHER,poptpl[0],poptpl[1])
...
plottacutoff(1,1.5,2080)
plottacutoff(1,1.5,200080)
plt.xscale('log')
plt.yscale('log')
plt.xlim(300,4000)
plt.xlabel('Energy [GeV]')
plt.ylabel('E dN/dE (arbitrary units)')
plt.legend()
plt.show()

print ((30*(6+9+9+6+6)+28*3+27*(9+9+6+9)+29*9)/81.)*110./30.
