...
if os.path.exists(savedir+CutoffTeVe+'final'+str(Brms)+'.txt'):
    a=np.loadtxt(savedir+CutoffTeVe+'final'+str(Brms)+'.txt')
else:
    a=[]
    a=np.loadtxt(savedirtmp+CutoffTeVe+'data_final0'+str(Brms)+'.txt')
    for i in range (1,int(nfile-1)):
        filename=savedirtmp+CutoffTeVe+'data_final'+str(i)+str(Brms)+'.txt'
        if os.path.exists(filename):
            s=np.loadtxt(savedirtmp+CutoffTeVe+'data_final'+str(i)+str(Brms)+'.txt')
...
py=(a[9])
pz=(a[10])
xarr=px/np.sqrt(py**2+pz**2)
yarr=py/np.sqrt(px**2+pz**2)
zarr=pz/np.sqrt(px**2+py**2)
theta=np.arctan(px/np.sqrt(py**2+pz**2))#*(np.pi/180)
phi=np.arctan(pz/py)#*(np.pi/180)
plt.hist2d(np.arctan(xarr)*180/np.pi,np.arctan(yarr)*180/np.pi,bins=1800,range=[[-90,90],[-90,90]],cmin=1)#,norm=LogNorm())
#plt.hist2d(theta,phi,bins=1800,range=[[-90,90],[-90,90]],cmin=1)
#plt.clim(1,2*1e5)
plt.xlim(-10,10)
plt.ylim(-10,10)
plt.xlabel('Theta [Degrees]')
