...
# Funzione che restituisce l'indice dei fotoni osservati da pixel di dimensioni radiusedg
def filtrapsf (a, radiusdeg): 
    idxphot=idxphot=np.sqrt(a[5]**2+a[6]**2)<866*2*degtorad(radiusdeg)
    return idxphot
...
if os.path.exists(savedir+CutoffTeV+'final'+str(Brms)+'.txt'):
    ...
    a=[]
    a=np.loadtxt(savedirtmp+CutoffTeV+'data_final0'+str(Brms)+'.txt', encoding='us-ascii')
